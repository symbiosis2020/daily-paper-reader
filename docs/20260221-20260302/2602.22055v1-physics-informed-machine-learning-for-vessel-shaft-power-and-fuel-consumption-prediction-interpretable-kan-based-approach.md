...
tags: ["query:sr"]
score: 8.0
evidence: 用于船舶功率和燃料预测的物理信息KAN
tldr: 本研究针对船舶轴功率和燃油消耗预测，提出了一种物理信息驱动的柯尔莫哥洛夫-阿诺德网络（PI-KAN）。该方法结合了可解释的单变量特征变换、物理约束损失函数和无泄漏链式预测流程，旨在解决传统物理模型适应性差和纯数据驱动模型缺乏物理一致性的问题。通过对五艘货轮的实测数据验证，PI-KAN在预测精度上显著优于传统多项式和神经网络模型，并成功还原了航速与功率的立方关系等物理规律，为船舶性能监控提供了兼具精度与可解释性的决策支持工具。
selection_source: fresh_fetch
motivation: 旨在解决传统物理模型在复杂海况下适应性不足，以及纯数据驱动模型在船舶性能预测中缺乏物理可解释性和一致性的问题。
method: 提出PI-KAN模型，通过集成单变量特征变换、物理信息损失函数和链式预测架构，将领域知识嵌入深度学习网络。
result: 在五艘货轮的实验中，该模型在轴功率和油耗预测的各项指标上均优于基准模型，并成功还原了航速与功率的立方关系等物理规律。
conclusion: PI-KAN成功实现了高精度预测与物理可解释性的平衡，为船舶性能监测和运营决策提供了稳健的技术支持。
---
