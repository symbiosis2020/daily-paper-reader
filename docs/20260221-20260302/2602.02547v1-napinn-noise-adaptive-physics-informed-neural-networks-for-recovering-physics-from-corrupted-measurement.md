...
tags: ["query:sr"]
score: 8.0
evidence: 从损坏的测量数据中恢复物理规律
tldr: 针对物理信息神经网络（PINN）在处理复杂噪声和异常值时性能下降的问题，本文提出了噪声自适应物理信息神经网络（naPINN）。该方法通过嵌入基于能量的模型来学习预测残差的潜在分布，并利用可训练的可靠性门自适应过滤高能量数据点。实验证明，naPINN在多种偏微分方程基准测试中表现优异，能够有效隔离异常值并在严重数据损坏下准确重建物理动力学。
selection_source: fresh_fetch
motivation: 传统的PINN在面对非高斯噪声和大量异常值时鲁棒性不足，且通常需要预验的噪声分布知识。
method: 引入基于能量的模型学习残差分布，结合可靠性门过滤异常数据，并使用拒绝成本正则化防止误删有效数据。
result: 在多种受非高斯噪声和异常值干扰的偏微分方程任务中，naPINN的性能显著优于现有的鲁棒PINN基准方法。
conclusion: naPINN提供了一种无需先验噪声知识即可从严重损坏的测量数据中稳健恢复物理规律的有效框架。
---
