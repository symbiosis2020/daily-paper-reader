...
## 摘要
求解偏微分方程 (PDE) 是众多科学和工程学科的基础。一个常见的挑战源于求解 PDE 族，其特点是具有相同的数学结构，但在特定参数上有所不同。传统的数值方法（如有限元法）需要独立求解 PDE 族中的每个实例，这会产生巨大的计算成本。另一方面，虽然机器学习 PDE 求解器的最新进展在计算速度和精度方面表现出色，但其固有的“黑盒”性质带来了相当大的局限性。这些方法主要产生数值近似，因此缺乏解析表达式所提供的关键可解释性，而解析表达式对于深入的科学洞察至关重要。为了解决这些局限性，我们提出了一种用于 PDE 族求解的神经辅助多任务符号 PDE 求解器框架，称为 NMIPS。特别地，我们采用多因素优化来同时发现 PDE 的解析解。为了提高计算效率，我们设计了一种仿射迁移方法，通过在 PDE 族中迁移学习到的数学结构，避免了从头开始求解每个 PDE。多个案例的实验结果表明，与现有基准相比，该方法取得了显著改进，在提供可解释解析解的同时，精度提高了约 35.7%。
